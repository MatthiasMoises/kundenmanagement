...
$get_news = mysql_query("SELECT id, author, text, DATE_FORMAT (date, '%d.%m.%Y %H:%i:%s') AS date FROM admin_news WHERE 1 ORDER BY id DESC") or die ("SELECT news error. ".mysql_error());
...
function deleteNews(id) {
		...
		if (confirm("Eintrag wirklich löschen?"))
		{
			...
			new Ajax.Updater("","admin_news_server.php", {
...
if ($news_count > 0)
{
    while ($row_news = mysql_fetch_assoc($get_news))
    {
		...
		if ($_SESSION['benutzername'] == $author && $author == "admin")
