...
$rRechnung = mysql_query("SELECT DATE_FORMAT (mahnungsdatum, '%d.%m.%Y') AS rech_date, kundennachricht, hat_stunden, hat_artikel, rabatt_prozent, rabatt_betrag, skonto_prozent, skonto_betrag, mwst_prozent, mwst_betrag, endbetrag, bezahlt, DATE_FORMAT (faelligkeitsdatum, '%d.%m.%Y') AS faelligkeitsdatum, editierbar FROM mahnungen WHERE mahnungsnr = '$acc_code'") or die("MySQL select demand note data error. ".mysql_error());

$rKunde = mysql_query("SELECT k.* FROM kunden k, mahnungen m WHERE m.kdnr = k.id AND m.mahnungsnr = '$acc_code'") or die("MySQL select customer error. ".mysql_error()); 
...
while ($row_rechnung = mysql_fetch_assoc($rRechnung))
{
	...
	$db_due_date = $row_rechnung['faelligkeitsdatum'];
...
if ($f_code == "all")
	$base = "show_demand_notes.php";

$folderdate = date("Y-m");

$folder = "pdf/mahnungen_pdf/".$folderdate;
...
{      
	...
	$rStunden = mysql_query("SELECT b.vorname, DATE_FORMAT (s.st_datum, '%d.%m.%Y') AS st_datum, s.ma_name, s.arbeit_art, s.zeit_stunden, s.euro_st, s.euro_st_gesamt FROM m_stunden s, benutzer b WHERE mahnungsnr = '$acc_code' AND s.ma_name = b.name ORDER BY s.st_datum, s.arbeit_art ASC") or die ("MySQL select m stunden error. ".mysql_error());
...
{  
	...
	$rArtikel = mysql_query("SELECT a.id, a.bezeichnung, a.preis_netto, ma.art_menge, ma.einzelpreis, ma.gesamtpreis_artikel FROM artikel a, mahnungen m, m_artikel ma WHERE m.mahnungsnr = '$acc_code' AND m.mahnungsnr = ma.mahnungsnr AND ma.artnr = a.id ORDER BY a.bezeichnung ASC") or die("MySQL select artikel error. ".mysql_error());
...
$pdf->SetLeftMargin(0);
$pdf->Cell(0,0,"Mahnung vom ".$db_rechnungsdatum,0,1,'L'); // Betreff
$pdf->Ln(8);
$pdf->SetFont('Arial','',9);
if ($db_message != "") {
...
$pdf->Cell(0,0,"Mahnungsnummer: ".$acc_code,0,1,'R');
$pdf->Ln(4.2);
...
$pdf->Ln(3);	
$pdf->SetFont('Arial','B',9);
$pdf->Cell(0,0,"Mahnungsbetrag: ".$pdf->setRechnungsbetrag($db_endbetrag)." EUR",0,0,'R');
$pdf->SetFont('Arial','',9);
$pdf->Ln(6);
...
$pdf->Cell(0,0,MAHNTEXT." ".$db_due_date,0,0,'C'); $pdf->Ln(6);
$pdf->SetFont('Arial','B',8);	
$pdf->Cell(0,0,INFO_MA_TXT,0,0,'C'); $pdf->Ln(4);
//Arial italic 8
$pdf->SetFont('Arial','',8);
// Footer text
$pdf->Cell(0,0,FOOTER_MA_1,0,0,'C'); $pdf->Ln(4);
$pdf->Cell(0,0,FOOTER_MA_2,0,0,'C'); $pdf->Ln(4);
$pdf->Cell(0,0,FOOTER_MA_3,0,0,'C'); $pdf->Ln(6);

//Output
$pdf->Output($pdfdatei,$m_code);

echo "<p>";
echo "Mahnung Nr. ".$acc_code." erfolgreich im Ordner ".$folder." gespeichert!";
echo "</p>";
