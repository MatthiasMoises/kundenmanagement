...
$base = "show_offers.php";

$rRechnung = mysql_query("SELECT DATE_FORMAT (angebotsdatum, '%d.%m.%Y') AS ang_date, kundennachricht, hat_stunden, hat_artikel, rabatt_prozent, rabatt_betrag, skonto_prozent, skonto_betrag, mwst_prozent, mwst_betrag, endbetrag, bezahlt, editierbar, ist_auftragsbestaetigung FROM angebote WHERE angebotsnr = '$acc_code'") or die("MySQL select offer data error. ".mysql_error());

$rKunde = mysql_query("SELECT k.* FROM kunden k, angebote an WHERE an.kdnr = k.id AND an.angebotsnr = '$acc_code'") or die("MySQL select customer error. ".mysql_error()); 

while ($row_rechnung = mysql_fetch_assoc($rRechnung))
{
	$db_rechnungsdatum = $row_rechnung['ang_date'];
...
$folder = "pdf/angebote_pdf/".$folderdate;
...
function Header()
{
	...
	$this->SetY(38);
    	//Info
	$this->Cell(0,0,utf8_decode("Ihr Fachgeschäft für:"),0,0);
	$this->Ln(5);
	$this->Cell(0,0,utf8_decode("Sat-Anlagen, Telefonanlagen, Gebäudeautomation, Verkauf und Reparatur aller Klein- u. Großgeräte, Sanitärinstallation"),10,0);
...
{      
	...
	$rStunden = mysql_query("SELECT b.vorname, DATE_FORMAT (s.st_datum, '%d.%m.%Y') AS st_datum, s.ma_name, s.arbeit_art, s.zeit_stunden, s.euro_st, s.euro_st_gesamt FROM a_stunden s, benutzer b WHERE angebotsnr = '$acc_code' AND s.ma_name = b.name ORDER BY s.st_datum, s.arbeit_art ASC") or die ("MySQL select a stunden error. ".mysql_error());
...
{       
	...
	$rArtikel = mysql_query("SELECT a.id, a.bezeichnung, a.preis_netto, aa.art_menge, aa.einzelpreis, aa.gesamtpreis_artikel FROM artikel a, angebote an, a_artikel aa WHERE an.angebotsnr = '$acc_code' AND an.angebotsnr = aa.angebotsnr AND aa.artnr = a.id ORDER BY a.bezeichnung ASC") or die("MySQL select artikel error. ".mysql_error());
...
function setNewEndbetragNetto($db_rabatt_betrag) {
	$this->endbetrag_netto -= $db_rabatt_betrag;
	return $this->endbetrag_netto;
}
...
$pdf->SetLeftMargin(0);
$pdf->Cell(0,0,"Angebot vom ".$db_rechnungsdatum,0,1,'L'); // Betreff
$pdf->Ln(8);
$pdf->SetFont('Arial','',9);
if ($db_message != "") {
...
$pdf->Cell(0,0,"Angebotsnummer: ".$acc_code,0,1,'R');
$pdf->Ln(4.2);
...
$pdf->Line(150,$y,200,$y);
$pdf->Ln(3);
$pdf->Cell(0,0,"Angebotspreis: ".$pdf->rundeBetrag($pdf->setNewEndbetragNetto($db_rabatt_betrag))." EUR",0,0,'R');
$pdf->Ln(5);
...
$pdf->Cell(0,0,"Angebotsbetrag: ".$pdf->setRechnungsbetrag($db_endbetrag)." EUR",0,0,'R');
$pdf->SetFont('Arial','',9);
$pdf->Ln(6);
$pdf->SetFont('Arial','I',6);
$pdf->Cell(0,0,"(".PAYING_INFO.")",0,0,'R');
$pdf->SetFont('Arial','',9);
...
if ($db_auftragsbestaetigung == 0) {
	$pdf->Cell(0,0,ANGEBOTSTEXT,0,0,'C'); $pdf->Ln(6);
	$pdf->SetFont('Arial','B',8);	
	$pdf->Cell(0,0,FOOTER_AN_1,0,0,'C'); $pdf->Ln(4);
	//Arial italic 8
	$pdf->SetFont('Arial','',8);
	// Footer text
	$pdf->Cell(0,0,FOOTER_AN_2,0,0,'C'); $pdf->Ln(4);
	$pdf->Cell(0,0,FOOTER_AN_3,0,0,'C'); $pdf->Ln(4);
}
else {
	$y=$pdf->GetY()-5;
	$pdf->Line(5,$y,80,$y);	
	$pdf->Cell(0,0,"Ort, Datum, Unterschrift Auftraggeber",0,0,'L'); $pdf->Ln(6);
	$pdf->Line(130,$y,205,$y);
	$y=$pdf->GetY()-5;
	$pdf->SetY($y);
	$pdf->Cell(0,0,"Ort, Datum, Unterschrift Auftragnehmer",0,0,'R'); $pdf->Ln(6);
}
...
echo "<p>";
echo "Angebot Nr. ".$acc_code." erfolgreich im Ordner ".$folder." gespeichert!";
echo "</p>";
