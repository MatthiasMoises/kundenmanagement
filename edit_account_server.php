...
switch($cmd) {
	...
	*/
	...
	case 'toTrashcan':
		...
		$trash_account = sprintf("UPDATE rechnungen SET im_papierkorb = 'y' WHERE rechnungsnr = '%s'",$acc_nr);
		
		$query_account = mysql_query($trash_account) or die ("MySQL account to trashcan query error. ".mysql_error());
		
		$log_date = date("Y-m-d H:i:s");
		$log_txt = $_SESSION['benutzername']." hat Rechnung Nr. $rechnungsnr in den Papierkorb verschoben.";
		$log_entry = mysql_query("INSERT INTO log (date, log_txt) VALUES ('$log_date','$log_txt')") or die ("MySQL INSERT log error. ".mysql_error());
	...
	case 'restoreAccount':
		...
		$restore_account = sprintf("UPDATE rechnungen SET im_papierkorb = 'n' WHERE rechnungsnr = '%s'",$acc_nr);
		
		$query_account = mysql_query($restore_account) or die ("MySQL restore account query error. ".mysql_error());
		
		$log_date = date("Y-m-d H:i:s");
		$log_txt = $_SESSION['benutzername']." hat Rechnung Nr. $rechnungsnr wiederhergestellt.";
		$log_entry = mysql_query("INSERT INTO log (date, log_txt) VALUES ('$log_date','$log_txt')") or die ("MySQL INSERT log error. ".mysql_error());
	
		break;	
		
	case 'clearTrashcan':
		$select = mysql_query("SELECT rechnungsnr, hat_stunden, hat_artikel FROM rechnungen WHERE im_papierkorb = 'y'");
		
		while ($row_trash = mysql_fetch_assoc($select)) {
			$trash_acc_nr = $row_trash['rechnungsnr'];
			$trash_hat_artikel = $row_trash['hat_artikel'];
			$trash_hat_stunden = $row_trash['hat_stunden'];
			
			if ($trash_hat_stunden == 1 && $trash_hat_artikel == 1)
			{
				$delete = "DELETE r.*, rs.*, ra.* FROM rechnungen r, r_stunden rs, r_artikel ra WHERE r.rechnungsnr = '$trash_acc_nr' AND ra.rechnungsnr = r.rechnungsnr AND rs.rechnungsnr = r.rechnungsnr";
			}
			else if ($trash_hat_stunden == 1 && $trash_hat_artikel == 0)
			{
				$delete = "DELETE r.*, rs.* FROM rechnungen r, r_stunden rs WHERE r.rechnungsnr = '$trash_acc_nr' AND rs.rechnungsnr = r.rechnungsnr";	
			}
			else if ($trash_hat_stunden == 0 && $trash_hat_artikel == 1)
			{
				$delete = "DELETE r.*, ra.* FROM rechnungen r, r_artikel ra WHERE r.rechnungsnr = '$trash_acc_nr' AND ra.rechnungsnr = r.rechnungsnr";	
			}
			else {
				echo "error.";	
			}
			
			mysql_query($delete) or die ("MySql clear trashcan query error. ".mysql_error());	
		}
	
		$log_date = date("Y-m-d H:i:s");
		$log_txt = $_SESSION['benutzername']." hat den Papierkorb geleert!";
		$log_entry = mysql_query("INSERT INTO log (date, log_txt) VALUES ('$log_date','$log_txt')") or die ("MySQL INSERT log error. ".mysql_error());
	...
	case 'createDemandNote':
		...
		$insert_mahnung = mysql_query("INSERT INTO mahnungen (mahnungsnr,mahnungsdatum,kundennachricht,hat_stunden,hat_artikel,rabatt_prozent,rabatt_betrag,skonto_prozent,skonto_betrag,mwst_prozent,mwst_betrag,endbetrag,kdnr,bezahlt,faelligkeitsdatum,editierbar) VALUES ('$db_rechnungsnr','$db_rechnungsdatum','$kundennachricht','$hat_stunden','$hat_artikel','$db_rabatt_prozent','$db_rabatt_betrag','$db_skonto_prozent','$db_skonto_betrag','$db_mwst_prozent','$db_mwst_betrag','$db_endbetrag','$db_kdnr','0','$due_date','1')") or die("MySQL insert mahnungen error. ".mysql_error());
		...
		case 'changePrintState':
			...
			$update = mysql_query("UPDATE rechnungen SET gedruckt = '$newState' WHERE rechnungsnr = '$acc_nr'") or die ("MySQL update rechnungen error. ".mysql_error());	
			}
			
			if ($update) {
				
				if ($newState == 1)
					$log_txt = $_SESSION['benutzername']." hat Rechnung Nr. $acc_nr auf \"Gedruckt\" gesetzt.";
				else
					$log_txt = $_SESSION['benutzername']." hat Rechnung Nr. $acc_nr auf \"Noch nicht gedruckt\" gesetzt.";
