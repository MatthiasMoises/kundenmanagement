...
$rRechnung = mysql_query("SELECT id, angebotsnr, DATE_FORMAT (angebotsdatum, '%d.%m.%Y') AS ang_date, kundennachricht, hat_stunden, hat_artikel, rabatt_prozent, rabatt_betrag, skonto_prozent, skonto_betrag, mwst_prozent, endbetrag, kdnr, bezahlt, editierbar, ist_auftragsbestaetigung FROM angebote WHERE angebotsnr = '$rNr'") or die("MySQL select offer data error. ".mysql_error());

$rKunde = mysql_query("SELECT k.* FROM kunden k, angebote an WHERE an.kdnr = k.id AND an.angebotsnr = '$rNr'") or die("MySQL select customer error. ".mysql_error()); 

$rArtikel = mysql_query("SELECT a.id, a.bezeichnung, a.preis_netto, aa.art_menge, aa.gesamtpreis_artikel FROM artikel a, angebote an, a_artikel aa WHERE an.angebotsnr = '$rNr' AND an.angebotsnr = aa.angebotsnr AND aa.artnr = a.id ORDER by aa.id ASC") or die("MySQL select artikel error. ".mysql_error());

$rStunden = mysql_query("SELECT * FROM a_stunden WHERE angebotsnr = '$rNr' ORDER BY id ASC") or die ("MySQL select a stunden error. ".mysql_error());
...
{
	die("<span style='color:red'>Keine Angebotsnummer angegeben!</span>");
}

if ($db_editierbar == '0')
{
	die("<span style='color:red'>Dieses Angebot kann nicht mehr bearbeitet werden!</span>");	
}
?>
	...
	{	
		new Ajax.Request("edit_offer_server.php",
		{
			parameters: {
                ...
                'angebots_id'    : db_rechnungs_id,
                'angebotsnr_alt' : db_rechnungsnr_alt,
		  ...
		  if ($('kd_nr').value != "" && $('nachname').value != "" && $('vorname').value != "")
		  {
			  	...
			  	$('save_account').disabled = true;
					...
					if (stundenString != "" || artikelString != "")
					{
						...
						'angebots_id'	 	  : $('rechnungs_id').value,
						'angebotsnr'		  : $('rechnungsnr').value,
...
<div>
    ...
    <input type="checkbox" id="auftragsbestaetigung" name="auftragsbestaetigung" <?php if ($db_auftragsbestaetigung == 1) echo 'checked="checked"'; ?> />
</div>
...
<div style="float:left;">
<p>
<button style="margin-left:500px;" id="save_account" name="save_account" onClick="javascript:check_account_nr_update()">Angebot speichern</button>
</p>
</div>
