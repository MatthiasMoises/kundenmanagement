...
if (isset($_POST['login']))
{
	...
	{
   if (preg_match('#^[a-z‰ˆ¸]+$#i', $passwort)) {
      $security = false;
   }
   else {
      $security = true;
		...
		if ($count = mysql_num_rows($select) > 0)
		{
			...
			if (($passwort === $db_pass) && $db_gesperrt == 0)
			{
           ...
           header("Location: main.php?notification=true");
        }
      }
			else {
...
{
	echo "<p style='color:red;'>Session Timeout - Ihre Sitzung wurde aus sicherheitsgr√ºnden beendet. Bitte erneut einloggen!</p>";	
}
...
<title>FormerCompany Kundenmanagement - Benutzerlogin</title>
...
<a href="admin/admin.php">Zum Administratorbereich</a>
