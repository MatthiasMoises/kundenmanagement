...
if (isset($_GET['bt_search']))
{
	...
	// count records
	$record_count = mysql_num_rows(mysql_query("SELECT r.id, r.rechnungsnr, DATE_FORMAT (r.rechnungsdatum, '%d.%m.%Y') AS rech_date, r.kdnr, r.bezahlt, r.editierbar, r.hat_stunden, r.hat_artikel, DATE_FORMAT (r.bezahlt_datum, '%d.%m.%Y') AS bez_date, r.gedruckt, k.nachname, k.vorname FROM rechnungen r, kunden k WHERE r.kdnr = k.id AND r.im_papierkorb = 'n'"));
	...
	// display data
	$query = "SELECT r.id, r.rechnungsnr, DATE_FORMAT (r.rechnungsdatum, '%d.%m.%Y') AS rech_date, r.kdnr, r.bezahlt, r.editierbar, r.hat_stunden, r.hat_artikel, DATE_FORMAT (r.bezahlt_datum, '%d.%m.%Y') AS bez_date, r.gedruckt, k.nachname, k.vorname FROM rechnungen r, kunden k WHERE r.kdnr = k.id AND r.im_papierkorb = 'n' ORDER BY id ASC LIMIT $start, $per_page";
}
else if ($orderBy != NULL && $sort != NULL) {
	$query = "SELECT r.id, r.rechnungsnr, DATE_FORMAT (r.rechnungsdatum, '%d.%m.%Y') AS rech_date, r.kdnr, r.bezahlt, r.editierbar, r.hat_stunden, r.hat_artikel, DATE_FORMAT (r.bezahlt_datum, '%d.%m.%Y') AS bez_date, r.gedruckt, k.nachname, k.vorname FROM rechnungen r, kunden k WHERE r.kdnr = k.id AND r.im_papierkorb = 'n' ORDER BY $orderBy $sort";	
}
else {
	$query = "SELECT r.id, r.rechnungsnr, DATE_FORMAT (r.rechnungsdatum, '%d.%m.%Y') AS rech_date, r.kdnr, r.bezahlt, r.editierbar, r.hat_stunden, r.hat_artikel, DATE_FORMAT (r.bezahlt_datum, '%d.%m.%Y') AS bez_date, r.gedruckt, k.nachname, k.vorname FROM rechnungen r, kunden k WHERE r.kdnr = k.id AND r.im_papierkorb = 'n' ORDER BY id ASC";	
}
...
<table id="account_table" border="1">
	<thead>
		...
		while ($row = mysql_fetch_assoc($select))
		{
			...
			echo "	<td align='center'><a href='javascript:toTrashcan({$rechnung->getRechnungsNr()})'><img src='img/attach_delete.gif' alt='delete' title='Rechnung in den Papierkorb verschieben' /></a></td>";
